					...
					if (item.isFormField()) { //普通表单字段
						...
						if (fileName != null && !fileName.equals("")) {
							...
							if (!fileType.contains(ext)) { //判断文件类型是否在允许范围内
								System.out.print("上传失败，文件类型只能是gif、bmp、jpg <br/>");
							} else {
								...
								System.out.print("上传成功后的文件名是：" + imgName
										+ "<br/>");
								System.out.print("上传成功后的文件地址是：" + imgURL
										+ "<br/>");
								PubBiz pubBiz = new PubBizImpl();
								pubBiz.insert(pubTitle, pubType, pubUser, pubTime, pubContent,imgName,imgURL);
